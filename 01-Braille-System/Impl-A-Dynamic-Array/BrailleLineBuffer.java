...
public class BrailleLineBuffer implements LineBuffer {
    ...
    public void expandBuffer() {
        ...
        int newSize = (int) Math.ceil(size() * growthFactor);
        newSize = Math.max(newSize, size() + 1); // just to be safe it's +1
    ...
    public void reduceBuffer() {
        if ((double) count / size() >= reductionRatio) {
            return;
        }
    ...
    public boolean insert(int cursorPosition, char[][] bitmap) {
        if (cursorPosition > count || cursorPosition < 0) {
            return false;
        }

        expandBuffer();
        System.arraycopy(buffer, cursorPosition, buffer, cursorPosition + 1, count - cursorPosition); // shifting to the right
        buffer[cursorPosition] = bitmap; // adding new braille char
        count++;
        return true;
    }
    ...
    public boolean delete(int cursorPosition) {
        ...
        System.arraycopy(buffer, cursorPosition + 1, buffer, cursorPosition, size() - cursorPosition - 1);
        count--;
        reduceBuffer();
        return true;
    }
    ...
    public String[] renderScanlines(int spacing) {
        ...
        String space = "";
        for (int i = 0; i < spacing; i++) { // gets spacing into a string
            space += " ";
        }
