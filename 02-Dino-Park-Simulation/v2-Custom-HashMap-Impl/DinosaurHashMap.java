...
public class DinosaurHashMap implements HashMap {
    ...
    private BucketList[] reorganizeBuckets(int newCapacity) {
        if (newCapacity <= 0) {
            newCapacity = DEFAULT_NUM_OF_BUCKETS;
        }

        BucketList[] newBuckets = new BucketList[newCapacity];
        DinosaurHashMapIterator iterator = this.iterator();

        while (iterator.hasNext()) {
            Dinosaur dinosaur = iterator.next();
    ...
    public Dinosaur put(Dinosaur dinosaur) {
        int index = dinosaur.getDNA().hashCode() % this.buckets.length;
        if (buckets[index] == null) {
            buckets[index] = new DinosaurBucketList();
        }

        Dinosaur toReturn = this.buckets[index].store(dinosaur);

        if (toReturn == null) {
            this.size++;
        }

        if (this.buckets[index].size() > maxBucketSize) {
    ...
    public Dinosaur get(DinosaurDNA dna) {
        int index = dna.hashCode() % this.buckets.length;
        if (this.buckets[index] == null) {
            return null;
        }
        return this.buckets[index].find(dna);
    }
    ...
    public Dinosaur remove(DinosaurDNA dna) {
        ...
        Dinosaur removed = this.buckets[index].remove(dna);
        if (removed != null) {
            this.size--;
        }
        return removed;
