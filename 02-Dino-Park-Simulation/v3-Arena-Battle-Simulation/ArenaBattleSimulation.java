...
public class ArenaBattleSimulation implements BattleSimulation {
    ...
    public Dinosaur executeFight(Dinosaur dinoA, Dinosaur dinoB) {
        ...
        FightingBehavior planA = dinoA.getFightingBehavior();
        FightingBehavior planB = dinoB.getFightingBehavior();
        int finalResult = 0;

        for (int round = 0; round < ArenaFightingBehavior.BATTLEPLAN_SIZE; round++) {
            Action actionA = planA.getPlannedAction(round);
            Action actionB = planB.getPlannedAction(round);
            int roundResult = this.executeAction(actionA, actionB);
            this.logRound(round, actionA, actionB, roundResult);
            finalResult += roundResult;
        }
        this.logResult(finalResult, dinoA, dinoB);
    ...
    public int executeAction(Action actionA, Action actionB) {
        ...
        return rules[actionA.ordinal()][actionB.ordinal()];
    }
