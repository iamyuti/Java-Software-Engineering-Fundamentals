...
public class ArenaFightingBehavior implements FightingBehavior {
    ...
    public ArenaFightingBehavior(DinosaurDNA dna) {
        ...
        int genSequence = dna.getGeneticCode() >> 4;
        decodeBehavior(genSequence);
    }
    ...
    public void decodeBehavior(int geneSequence) {
        ...
        for (int i = 0; i < BATTLEPLAN_SIZE; i++) {
            byte gene = (byte) ((geneSequence >> (2 * i)) & 0b11);
            battlePlan[i] = decodeActionGene(gene);
        }
    }
    ...
    public Action getPlannedAction(int index) {
        ...
        if (index < battlePlan.length && index >= 0) {
