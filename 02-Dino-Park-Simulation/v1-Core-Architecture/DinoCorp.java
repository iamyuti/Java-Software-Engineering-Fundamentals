...
public class DinoCorp {
    ...
    public boolean processNextOrder(){
        ...
        String currentOrder = this.orders[this.currentOrderIndex];
        currentOrderIndex++;
        if (currentOrder.isBlank()) {
            return false;
        }
        char keyChar = currentOrder.charAt(0);

        if (keyChar == '#') {
            String factoryName = currentOrder.substring(1);
            return this.activateFactory(factoryName);
        } else if (Character.isLetter(keyChar)) {
            if (currentOrder.contains("!") && activeFactory != -1) {
                int startAt = currentOrder.indexOf("!") + 1;
                int dna = Integer.parseInt(currentOrder.substring(startAt));
                String name = currentOrder.substring(1, currentOrder.indexOf("!"));
                var dinosaur = this.getActiveFactory().create(dna, name);
                this.population = this.population.store(dinosaur);
                return true;
            }
        }
        return false;
    ...
    public void feed(Dinosaur.Food food){
        Dinosaur[] dinosaurs = this.population.flatten();
        for (var dinosaur : dinosaurs) {
            dinosaur.feed(food);
        }
    }
    ...
    public AbstractDinosaurFactory getActiveFactory(){
        if (this.activeFactory != -1) {
            return this.factories[this.activeFactory];
        } else {
            return null;
        }
    }
